        ...
        private string BuildCompetitionsCommand(Competitions c)
        {
            ...
            if (c.SendTo.Length <= 1)
            {
                ...
                string prefix = $"Declare @ContestNum int;" +
                                $" INSERT INTO Competitions VALUES ('{c.SendFrom}', '{c.SendTo[0]}','{c.Grid}', '{c.Keys}', '{c.Word}', '{c.Clues}', '{c.Legend}, {c.FromCountAnswer}, {c.ToCountAnswer}');" +
            ...
            else
            {
                ...
                {
                    str += Competitions(c.SendFrom, c.SendTo[i], c.Grid, c.Keys, c.Word, c.Clues, c.Legend, c.FromCountAnswer, c.ToCountAnswer, c.Notification.Type, c.Notification.Text, SQLFormat);
                }
                return str;
            }
        }
        private string Competitions(string sendFrom, string sendTo, string grid, string keys, string word, string clues, string legend, int fromCountAnswer, int toCountAnswer, string type, string text, string d)
        {
            return $"Declare @ContestNum int;" +
                                $"INSERT INTO Competitions VALUES ('{sendFrom}', '{sendTo[0]}','{grid}', '{keys}', '{word}', '{clues}', '{legend}','{fromCountAnswer}', '{toCountAnswer}');" +
                                $"select @ContestNum = SCOPE_IDENTITY()" +
                                $"INSERT INTO Notifications VALUES ('{sendFrom}', '{sendTo[0]}','{grid}', '{keys}', '{word}', '{clues}', '{legend}','{toCountAnswer}','{toCountAnswer}', {"NULL"},{"NULL"}, @ContestNum, {d}, {0}, {0});";
        }
